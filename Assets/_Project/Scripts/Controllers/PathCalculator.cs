...
public static class PathCalculator
{
        ...
        NpcSettings npcSettings)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            routeContext.timeToStart[i] = Vector3.Distance(allBalls[i].transform.position, startPos) / speed;
            
            for (int j = i + 1; j < n; j++)
            {
                float timeBetween = Vector3.Distance(allBalls[i].transform.position, allBalls[j].transform.position) 
                                                    / speed;
                routeContext.timeBetween[i, j] = timeBetween;
                routeContext.timeBetween[j, i] = timeBetween;
            }
        }
